...
import pandas as pd
import numpy as np
from data_cluster.kmeans import KMeansClassifier
...
def loadDataset():
    ...
    # 合并经纬度数据
    df_latlng = pd.read_csv("../data_file/latlng.csv", skiprows=[0], names=["id", "communityName", "lat", "lng"])
...
plt.savefig('../data_cluster/result/不同K值下的和方差折线图.jpg')
plt.show()
...
for i in range(k):
    ...
    mean_data = [mean_data_ for mean_data_ in mean_data if mean_data_ == mean_data_]
...
plt.savefig('../data_cluster/result/聚类结果-单价与建筑面积的散点图.jpg')
plt.show()
...
plt.savefig('../data_cluster/result/聚类结果-总价与建筑面积的散点图.jpg')
plt.show()
